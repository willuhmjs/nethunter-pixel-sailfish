...
/* main.c arms this with setup_timer(..., (unsigned long)rtwdev) before 4.14
 * and with timer_setup() after; the definition has to follow suit.
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
void rtw_tx_report_purge_timer(void *ctx)
{
	struct rtw_dev *rtwdev = ctx;
	struct rtw_tx_report *tx_report = &rtwdev->tx_report;
#else
