...
#ifdef RTW_HAS_BSS_CHANGED_MU_GROUPS
	memcpy(param.given_gid_tab, conf->mu_group.membership, 8);
	memcpy(param.given_user_pos, conf->mu_group.position, 16);
#else
	/* No MU group membership available from mac80211 - see compat.h. */
	memset(param.given_gid_tab, 0, 8);
	memset(param.given_user_pos, 0, 16);
#endif
