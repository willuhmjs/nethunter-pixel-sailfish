...
#if RTW_WIRELESS_VERSION_CODE >= KERNEL_VERSION(4, 11, 0)
		ieee80211_cqm_rssi_notify(vif, event, sig, GFP_KERNEL);
#else
		/* The rssi_level argument only arrived in 4.11; userspace just
		 * gets the threshold-crossing event without the level.
		 */
					     ...
					     struct rtw_rsvd_page *rsvd_pkt)
{
	...
	case RSVD_NULL:
	case RSVD_QOS_NULL:
		/* ieee80211_nullfunc_get() only grew its qos_ok argument in
		 * 4.20; before that it always returns a non-QoS Nullfunc,
		 * which is still a valid PS-Poll/PS-null template for the
		 * firmware's reserved page.
		 */
#if RTW_WIRELESS_VERSION_CODE >= KERNEL_VERSION(4, 20, 0)
		skb_new = ieee80211_nullfunc_get(hw, vif,
						 rsvd_pkt->type == RSVD_QOS_NULL);
#else
		skb_new = ieee80211_nullfunc_get(hw, vif);
#endif
		break;
